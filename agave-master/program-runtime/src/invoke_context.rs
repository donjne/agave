use {
    crate::{
        ...
        // These imports handle compute budget management - limits and cost tracking
        loaded_programs::{
            ProgramCacheEntry, ProgramCacheEntryType, ProgramCacheForTxBatch,
            ProgramRuntimeEnvironments,
        },
        // Program caching system for efficient program loading and execution
        stable_log, // Logging utilities for execution tracing
        sysvar_cache::SysvarCache, // Cached system variables for network state
    },
    solana_account::{create_account_shared_data_for_test, AccountSharedData},
    // Account data structures and test utilities
    solana_clock::Slot, // Blockchain slot numbering
    ...
    solana_sbpf::{
        ebpf::MM_HEAP_START, // BPF virtual machine heap start address
        error::{EbpfError, ProgramResult}, // BPF execution errors
        memory_region::MemoryMapping, // Memory layout for BPF programs
        program::{BuiltinFunction, SBPFVersion}, // Built-in function definitions
        vm::{Config, ContextObject, EbpfVm}, // Virtual machine configuration
    },
    // Complete BPF virtual machine implementation for program execution
    solana_sdk_ids::{
        bpf_loader, bpf_loader_deprecated, bpf_loader_upgradeable, loader_v4, native_loader, sysvar,
    },
    // Various program loader identifiers for different program types
    solana_stable_layout::stable_instruction::StableInstruction,
    // Instruction format that maintains ABI stability
    solana_svm_callback::InvokeContextCallback, // Callback interface for runtime interactions
    solana_svm_feature_set::SVMFeatureSet, // Feature flags for SVM functionality
    solana_timings::{ExecuteDetailsTimings, ExecuteTimings}, // Execution timing measurements
    solana_transaction_context::{
        IndexOfAccount, InstructionAccount, TransactionAccount, TransactionContext,
    },
    // Transaction and account management within execution context
    solana_type_overrides::sync::{atomic::Ordering, Arc},
    // Atomic operations and memory ordering for thread safety
    std::{
        alloc::Layout, // Memory layout specifications for allocations
        cell::RefCell, // Interior mutability for shared references
        fmt::{self, Debug}, // Formatting and debug traits
        rc::Rc, // Reference counting for shared ownership
    },
};

pub type BuiltinFunctionWithContext = BuiltinFunction<InvokeContext<'static>>;
// Creates a convenient alias for builtin functions that use InvokeContext
// The 'static lifetime indicates these functions don't hold references to context data
...
macro_rules! declare_process_instruction {
    ($process_instruction:ident, $cu_to_consume:expr, |$invoke_context:ident| $inner:tt) => {
        // This macro generates a standardized wrapper for builtin program instructions
        $crate::solana_sbpf::declare_builtin_function!(
            ...
            fn rust(
                ...
                // Standard BPF function signature with 5 arguments (unused here)
                _memory_mapping: &mut $crate::solana_sbpf::memory_region::MemoryMapping,
                // Memory mapping for BPF program access
            ) -> std::result::Result<u64, Box<dyn std::error::Error>> {
                // Inner function that contains the actual instruction logic
                fn process_instruction_inner(
                    $invoke_context: &mut $crate::invoke_context::InvokeContext,
                ) -> std::result::Result<(), $crate::__private::InstructionError>
                    $inner // User-provided instruction implementation

                // Consume compute units before executing instruction
                let consumption_result = if $cu_to_consume > 0 {
                    invoke_context.consume_checked($cu_to_consume)
                } else {
                    Ok(())
                };
                // Chain compute consumption with instruction execution
                consumption_result
                    .and_then(|_| {
                        process_instruction_inner(invoke_context)
                            .map(|_| 0) // Convert success to BPF return value (0)
...
impl ContextObject for InvokeContext<'_> {
    fn trace(&mut self, state: [u64; 12]) {
        // Add execution trace to the current syscall context
        self.syscall_context
            .last_mut() // Get the top stack frame
            .unwrap() // Panic if no frames (programming error)
            .as_mut() // Convert Option<SyscallContext> to Option<&mut SyscallContext>
            .unwrap() // Panic if None (syscall context not initialized)
            .trace_log
            .push(state); // Add trace state to log
    }

    fn consume(&mut self, amount: u64) {
        // BPF VM interface for consuming compute units
        // Note: This ignores overflow - BPF VM will handle budget exceeded separately
        let mut compute_meter = self.compute_meter.borrow_mut(); // Get mutable reference
        *compute_meter = compute_meter.saturating_sub(amount); // Subtract without underflow
    }

    fn get_remaining(&self) -> u64 {
        // Return current remaining compute units
        *self.compute_meter.borrow() // Immutable borrow and dereference
    }
}

#[derive(Clone, PartialEq, Eq, Debug)]
pub struct AllocErr; // Simple allocation error type
...
pub struct BpfAllocator {
    len: u64, // Total available memory in the allocation arena
    pos: u64, // Current position (next allocation offset)
}

impl BpfAllocator {
    pub fn new(len: u64) -> Self {
        Self { len, pos: 0 } // Initialize with zero position
    }

    pub fn alloc(&mut self, layout: Layout) -> Result<u64, AllocErr> {
        // Calculate padding bytes needed for proper alignment
        let bytes_to_align = (self.pos as *const u8).align_offset(layout.align()) as u64;
        
        // Check if allocation fits within available space
        if self.pos
            .saturating_add(bytes_to_align) // Add alignment padding
            .saturating_add(layout.size() as u64) // Add requested size
            <= self.len // Ensure within bounds
        {
            self.pos = self.pos.saturating_add(bytes_to_align); // Apply alignment
            let addr = MM_HEAP_START.saturating_add(self.pos); // Calculate virtual address
            self.pos = self.pos.saturating_add(layout.size() as u64); // Update position
            Ok(addr) // Return allocated address
        } else {
            Err(AllocErr) // Not enough space
        }
    }
}
pub struct EnvironmentConfig<'a> {
    pub blockhash: Hash, // Current block hash for transaction validation
    pub blockhash_lamports_per_signature: u64, // Fee calculation parameter
    epoch_stake_callback: &'a dyn InvokeContextCallback, // Stake information provider
    feature_set: &'a SVMFeatureSet, // Enabled runtime features
    sysvar_cache: &'a SysvarCache, // Cached system variables
}
...
impl<'a> EnvironmentConfig<'a> {
    pub fn new(
        ...
        // Simple constructor bundling all environment parameters
        Self {
            blockhash,
...
pub struct SyscallContext {
    pub allocator: BpfAllocator, // Memory allocator for this syscall context
    pub accounts_metadata: Vec<SerializedAccountMetadata>, // Account serialization info
    pub trace_log: Vec<[u64; 12]>, // Execution trace for debugging
}

#[derive(Debug, Clone)]
pub struct SerializedAccountMetadata {
    pub original_data_len: usize, // Original account data size
    pub vm_data_addr: u64, // Virtual address of account data in BPF VM
    pub vm_key_addr: u64, // Virtual address of account key
    pub vm_lamports_addr: u64, // Virtual address of lamports field
    pub vm_owner_addr: u64, // Virtual address of owner field
}
...
pub struct InvokeContext<'a> {
    ...
    // Borrowed reference to transaction state - allows modifications but prevents ownership transfer
    
    /// The local program cache for the transaction batch.
    pub program_cache_for_tx_batch: &'a mut ProgramCacheForTxBatch,
    // Cached loaded programs for efficient repeated access
    
    /// Runtime configurations used to provision the invocation environment.
    pub environment_config: EnvironmentConfig<'a>,
    // Bundled environment state (block hash, features, sysvars, etc.)
    
    /// The compute budget for the current invocation.
    compute_budget: SVMTransactionExecutionBudget,
    // Maximum allowed compute consumption
    
    /// The compute cost for the current invocation.
    execution_cost: SVMTransactionExecutionCost,
    // Estimated compute cost for planning
    
    /// Instruction compute meter, for tracking compute units consumed against
    /// the designated compute budget during program execution.
    compute_meter: RefCell<u64>,
    // Real-time compute consumption tracking with interior mutability
    
    log_collector: Option<Rc<RefCell<LogCollector>>>,
    // Optional logging collection for debugging
    ...
    // Stack of syscall contexts for nested program calls
    
    traces: Vec<Vec<[u64; 12]>>,
    // Collected execution traces from completed instructions
}
...
impl<'a> InvokeContext<'a> {
    ...
    pub fn new(
        ...
        Self {
            ...
            compute_budget,
            execution_cost,
            // Initialize compute meter with the budget limit
            compute_meter: RefCell::new(compute_budget.compute_unit_limit),
            execute_time: None,
            timings: ExecuteDetailsTimings::default(),
            syscall_context: Vec::new(), // Start with empty call stack
            traces: Vec::new(), // Start with no traces
        }
    }
    ...
    pub fn push(&mut self) -> Result<(), InstructionError> {
        // Get the instruction context for the current instruction being processed
        ...
        // Extract the program ID from the instruction context
        let program_id = instruction_context
            .get_last_program_key(self.transaction_context)
            .map_err(|_| InstructionError::UnsupportedProgramId)?;
        
        // Reentrancy protection: Check if we're already executing (stack height > 0)
        if self.transaction_context.get_instruction_context_stack_height() != 0 {
            // Check if any program in the current call stack matches our program_id
            ...
            // Check if the current (last) program in stack is the same as program_id
            let is_last = self
                .transaction_context
            ...
            // Prevent reentrancy unless the program is calling itself recursively
            if contains && !is_last {
                return Err(InstructionError::ReentrancyNotAllowed);
            }
        }

        // Add new syscall context frame (initially None for lazy initialization)
        self.syscall_context.push(None);
        // Push instruction context onto transaction stack
        self.transaction_context.push()
    }

    /// Pop a stack frame from the invocation stack
    fn pop(&mut self) -> Result<(), InstructionError> {
        // Remove the top syscall context from the stack
        if let Some(Some(syscall_context)) = self.syscall_context.pop() {
            // If the context was initialized, collect its trace log
            self.traces.push(syscall_context.trace_log);
        }
        // Pop the instruction context from transaction stack
...
/// Helper to prepare for process_instruction()
    ...
    pub fn prepare_instruction(
        ...
        // Process each account in the instruction
        for (instruction_account_index, account_meta) in instruction.accounts.iter().enumerate() {
            // Find the account's index in the transaction
            ...
            // Check if we've already processed this account (deduplication)
            ...
            {
                // Account already exists - merge privileges
        ...
        // Validate that privilege escalation is not occurring
    ...
    pub fn consume_checked(&self, amount: u64) -> Result<(), Box<dyn std::error::Error>> {
        let mut compute_meter = self.compute_meter.borrow_mut(); // Acquire mutable reference
        let exceeded = *compute_meter < amount; // Check if consumption would exceed budget
        *compute_meter = compute_meter.saturating_sub(amount); // Subtract without underflow
        if exceeded {
    ...
    pub fn mock_set_remaining(&self, remaining: u64) {
        *self.compute_meter.borrow_mut() = remaining; // Direct assignment for testing
    }
